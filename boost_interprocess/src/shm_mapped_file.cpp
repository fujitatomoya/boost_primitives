...
int main(int argc, char *argv[])
{
  ...
  // Define file names
  const char *FileName  = "./shm_mapped_file.bin";
  const std::size_t FileSize = 10000;

  if(argc == 1) { // Parent process executes this
    {  // Create a file
      file_mapping::remove(FileName); // make sure it does not exist before
      std::filebuf fbuf;
    ...
    struct file_remove
    {
      ...
      printf("(Parent Process)VA for mapped file is %p, and size is %zu\n", addr, size);

      // Write all the memory to 1 (occur pagefault to get physical pages)
    ...
    { // Open the file mapping and map it as read-only
      ...
      void * addr = region.get_address();
      std::size_t size = region.get_size();
      printf("(Child Process)VA for mapped file is %p, and size is %zu\n", addr, size);

      // Check that memory was initialized to 1
      const char *mem = static_cast<char*>(addr);
      for(std::size_t i = 0; i < size; ++i)
        if(*mem++ != 1)
          return EXIT_FAILURE; // Error checking memory
    }
